...
function createInbox() {
  ...
  if (arrayOfProjects.filter((project) => project.name == "Inbox").length == 0)
    arrayOfProjects.push(inbox);

  TodoButton(inbox);

  localStorage.setItem("projects", JSON.stringify(arrayOfProjects));
  arrayOfProjects.filter((project) => project.name == "Inbox").length > 0
...
function searchForTasks() {
  ...
  searchInput.addEventListener("keyup", () => {
    ...
    arrayOfProjects.forEach((project) => {
      project.arrayOfTodos.filter((todo) =>
        todo.name.indexOf(searchInput.value) > -1 && searchInput.value
          ? filtered.arrayOfTodos.push(todo)
          : 0
      );
...
function createAllTasksInProject(project, indexOfTodayTask, origin) {
  ...
  project.arrayOfTodos.forEach((todo) => {
    ...
    if (todo.date) {
      ...
      if (!indexOfTodayTask && isToday(new Date(todo.date))) {
        // time for today tasks
        taskTimeLeft.textContent = "today";
        taskDivRight.appendChild(taskTimeLeft);
      }
      if (origin == "upcoming" || isAfter(new Date(todo.date), new Date())) {
        //time for tasks in the past
    ...
    taskCheckbox.addEventListener("click", () => {
      deleteTask(
        project,
        project.arrayOfTodos.findIndex(
          (projectTodo) => projectTodo.name == todo.name
        )
      );
      deleteTask(
        arrayOfProjects.find((project) => project.name == todo.project),
        project.arrayOfTodos.findIndex(
          (projectTodo) => projectTodo.name == todo.name
        )
      );
...
function updateCurrentProject(project) {
  project.name
    ...
    : (document.querySelector(".project-name").textContent = project);
  project == "Search" || project.name == "Search"
    ? document.querySelector(".search-field").classList.remove("invisible")
    : document.querySelector(".search-field").classList.add("invisible");
}
...
function inputProjectName() {
  ...
  submitButtonForProjectName.classList.add(
    "projects__btn",
    "projects__btn--submit"
  );
